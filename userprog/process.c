...
bool load (const char *file_name, void (**eip) (void), void **esp) {
  ...
  //my code (need to free)
  char *token, *save_ptr;
  char *file_n = malloc(strlen(file_name) + 1);
  char *fileName_andArgs = malloc(strlen(file_name) + 1);
  strlcpy(file_n, file_name, strlen(file_name) + 1);
  strlcpy(fileName_andArgs, file_name, strlen(file_name) + 1);
  ...
  /* Set up stack. */
  if (!setup_stack (esp, fileName_andArgs))
    goto done;
